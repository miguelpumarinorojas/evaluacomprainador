...
<body>
    ...
    if (isset($_POST['btnRegistrar']) && !empty($_POST['CodigoProducto']) && !empty($_POST['DescripcionProducto'])) {
        ...
        $validarCodigo = $conn->prepare("SELECT * FROM productos WHERE codigo = ?");
        $validarCodigo->bind_param("s", $codigoProducto);
    ...
    ?>
            <div class='alert alert-danger notification alert-dismissible fade show text-center' role='alert' id='success-alert-v2'>
                El producto <?php echo $codigoProducto; ?> ya se encuentra registrado! <span class="material-icons align-bottom">error</span>
            </div>
        <?php } else {
            $validarCodigo->close();
    ...
    <nav aria-label="breadcrumb" class="bg-light py-2 px-3">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"><span class="material-icons align-bottom">category</span>Productos</li>
        </ol>
    </nav>
    <div class="container-fluid">
        <div class="row pt-2">
                ...
                <form action="" method="POST" class="needs-validation" autocomplete="off" enctype="multipart/form-data" novalidate>
                    <div class="card">
                        <div class="card-header bg-secondary text-white">
                            <span class="material-icons align-bottom">add</span> Registro de Productos
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-3">
