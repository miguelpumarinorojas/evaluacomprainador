...
<body>
    ...
    if (isset($_POST['btnRegistrar']) && !empty($_POST['NombreUsuario']) && !empty($_POST['EmailUsuario']) && !empty($_POST['PasswordUsuario'])) {
        ...
        $validarCodigo = $conn->prepare("SELECT * FROM usuarios WHERE email = ?");
        $validarCodigo->bind_param("s", $emailUsuario);
    ...
    ?>
            <div class='alert alert-danger notification alert-dismissible fade show text-center' role='alert' id='success-alert-v2'>
                El usuario <?php echo $emailUsuario; ?> ya se encuentra registrado! <span class="material-icons align-bottom">error</span>
            </div>
        <?php } else {
            $validarCodigo->close();
        ...
        ?>
            <div class='alert alert-success notification alert-dismissible fade show text-center' role='alert' id='success-alert-v2'>
                Usuario <?php echo $emailUsuario; ?> registrado exitosamente! <span class="material-icons align-bottom">done</span>
            </div>
    <?php }
    }
    ...
    <nav aria-label="breadcrumb" class="bg-light py-2 px-3">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"><span class="material-symbols-outlined align-bottom">account_circle</span>Usuarios</li>
        </ol>
    </nav>
    <div class="container-fluid">
        <div class="row pt-2">
                ...
                <form action="" method="POST" class="needs-validation" autocomplete="off" enctype="multipart/form-data" novalidate>
                    <div class="card">
                        <div class="card-header bg-secondary text-white">
                            <span class="material-icons align-bottom">add</span> Registro de usuarios
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col">
